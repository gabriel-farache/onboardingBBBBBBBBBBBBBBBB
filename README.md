...
[onboarding-schema.json](./src/main/resources/schemas/onboarding-schema.json ), run the following to create an
instance of the onboarding workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/onboarding -d@input.json
```

## Adding UI schema properties
You can add React UI schema properties to the input schema. See the 'ui:widget' example at [onboarding-schema.json](./src/main/resources/schemas/onboarding-schema.json ).
